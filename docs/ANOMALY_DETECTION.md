...
В таблице `precedent_queries` хранятся пять системных шаблонов с префиксом `★`. Аналитик открывает редактор PQL, нажимает «Загрузить» и видит готовые рецепты под типичные сценарии.

| Имя | Что ищет |
|---|---|
| `★ Объём: всплеск > 1.5×` | События с `volume_ratio(...) > 1.5`, отсортированы по убыванию |
| `★ Волатильность: рост > 2×` | События с `vol_ratio(...) > 2.0`, отсортированы по убыванию |
| `★ Возможный инсайд (CAR до t=0)` | `ABS(car(..., window_after => -1)) > 0.03` — движение цены ДО даты события |
| `★ Дивиденды LKOH: реакция цены` | JOIN с `DIVIDEND_ANNOUNCEMENT` + CAR на дату объявления |
| `★ Многофакторная оценка аномальности` | `CASE WHEN ... + CASE WHEN ... HAVING score >= 2` (см. пример выше) |

Имена с префиксом `★` защищены от перезаписи: `POST /api/precedents/queries` отклонит создание пользовательского рецепта с таким именем (409).
