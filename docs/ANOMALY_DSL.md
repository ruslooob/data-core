# Язык запросов для поиска аномалий (устаревший черновик)

> ⚠️ **Этот документ устарел.** Концепт переработан в **язык поиска прецедентов** (Precedent Query Language, PQL): язык стал общего назначения, поиск аномалий — один из частных случаев. Текущая спецификация языка живёт в [SPEC_PRECEDENT_LANGUAGE.md](SPEC_PRECEDENT_LANGUAGE.md).
>
> Этот файл оставлен как исторический черновик: в нём есть идеи (`SET`, `DEFINE`, агрегации, многофакторный скоринг), которые планируются после MVP в виде расширений PQL.
