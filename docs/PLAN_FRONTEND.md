...
### Шаг 2.3 — Layout с контейнером виджетов
- Убрать дефолтный контент Vite
- Создать компонент `WidgetContainer` — вертикальный стек с кнопкой «+ Добавить виджет»
- Кнопка вызывает меню выбора типа виджета (Price chart / Event study)
- Каждый виджет — `<Widget>` с заголовком, кнопкой закрытия, содержимым

**Критерий приёмки:**
- На странице кнопка «+ Добавить виджет»
- Можно добавить пустой виджет-плейсхолдер, удалить его
