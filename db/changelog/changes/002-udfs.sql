-- Auto-generated from scripts/init_postgres_udfs.py + init_postgres_backtest_udfs.py.
-- Liquibase прогоняет этот файл как один statement (splitStatements=false),
-- потому что внутри PL/Python-тел встречаются `;`. Каждое CREATE OR REPLACE
-- FUNCTION идемпотентно — runOnChange="true" в обёрточном XML.
